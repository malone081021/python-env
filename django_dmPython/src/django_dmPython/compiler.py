...
class SQLCompiler(compiler.SQLCompiler):
    def compile(self, node, select_format=False):
        ...
        if hasattr(node, 'lookup_name'):
            #f getattr(node, 'lookup_name') == 'exact':
            sql = sql.replace('%s','?')    
...
class SQLUpdateCompiler(compiler.SQLUpdateCompiler, SQLCompiler):
    def as_sql(self):
        ...
        """
        '''
        modified on 2019-7-31
        as_sql code implementation same with compiler.SQLUpdateCompiler.as_sql
        except "placeholder" value: changed '%s' to '?'
        '''
        self.pre_sql_setup()
